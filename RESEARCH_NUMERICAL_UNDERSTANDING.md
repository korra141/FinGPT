...
## Beyond Tokenization: Other Lenses on Numerical Understanding

Tokenization is one explanation, but it is not the whole story. Even if a number is a single token, the model may still not "understand" it. Additional angles:

### Embedding Geometry
Even with a clean single token, its embedding vector has no guaranteed relationship to numerically adjacent values. `$1.00` and `$1.01` may be unrelated vectors if they co-occurred in different contexts during training. You can probe this by extracting embeddings for a numeric range (e.g., `0.01`, `0.10`, `1.00`, `10.00`) and measuring cosine similarity. If the model has implicit numeric structure, magnitude-adjacent numbers should cluster — usually they don't, but numbers that frequently co-occur in financial text (e.g., common EPS values) sometimes do.

### Gradient-Based Saliency
Integrated gradients or SHAP on input embeddings tells you which tokens most influenced the output prediction — independent of tokenization. A number may be a clean single token but still have low saliency if the surrounding context words are responsible for the prediction.

### Probing Classifiers
Train a small linear probe on the model's hidden states at numeric token positions: can it recover the *magnitude* of the number from that hidden state? If not, the model has discarded quantitative information by the time it builds its contextual representation.

### Out-of-Distribution Numbers
Feed numbers the model almost certainly never saw in training — e.g., EPS of `$7,842.11` or a stock move of `+0.0003%`. If it still produces confident outputs, it is clearly pattern-matching on context, not reasoning from the value.

### Attention Attribution
Run inference with `output_attentions=True`. Compute the average attention weight placed on numeric tokens vs. non-numeric tokens (normalized by count). This is imperfect (attention ≠ importance) but gives a first-pass signal, especially when comparing base model vs. fine-tuned model.
...
**H3 (Tokenizer Effect):** Models using tokenizers with more consistent number tokenization (e.g., GPT-4's tiktoken) will show *marginally* better numerical sensitivity than models with aggressive subword splitting, but the gap will be small relative to the context-dominance effect.

**H4 (Fine-tuning Doesn't Fix It):** LoRA fine-tuning on financial text will improve domain-specific language understanding but will *not* improve numerical sensitivity, because the fine-tuning signal is at the sentence/label level and does not teach the model to reason about magnitude.

**H5 (Embedding Blindness):** Numeric token embeddings do not encode ordinal or magnitude relationships — numerically adjacent values will not be geometrically close in embedding space.
...
1. Create three versions of each test input:
   - **Original:** full text
   - **Numbers only:** replace all non-numeric tokens with a neutral filler (e.g., `the`)
...
**Expected result:** "Words only" performance ≈ "Original"; "Numbers only" performance ≈ random.
...
4. Compare: base model vs. LoRA fine-tuned model.

**Expected result (if H4 holds):** Fine-tuning will not increase relative attention on numeric tokens.

**Caveat:** Attention weights are not a reliable proxy for importance. Pair with gradient-based attribution (e.g., integrated gradients via `captum`) for stronger claims.

---

### Experiment 5: Breaking the Correlation — Number Normalization Preprocessing
**Goal:** If H1–H2 hold, can we improve numerical sensitivity by preprocessing numbers before tokenization?

**Approaches to test:**
- **Verbalization:** `0.08` → `eight cents` or `$0.08 (small miss)`. Forces the model to use language patterns for magnitude.
...
- **Structured tagging:** `<EPS_MISS magnitude=small>0.08</EPS_MISS>`. Tests whether explicit semantic structure helps.
- **Number-to-word expansion:** `142.50` → `one hundred forty two dollars and fifty cents`.
...
### Experiment 6: Embedding Geometry Probe
**Goal:** Test H5 — do numeric embeddings encode any notion of magnitude or order?

**Method:**
1. Extract input embeddings for a sweep of numeric tokens (e.g., `1`, `2`, ..., `100`, `0.1`, `1.0`, `10.0`).
2. Compute pairwise cosine similarity matrix.
3. Apply UMAP/t-SNE to visualize clustering.
4. Compare to randomly sampled non-numeric word embeddings as a baseline.

**Expected result (if H5 holds):** Numeric embeddings will not cluster by magnitude; they will be scattered relative to their frequency/co-occurrence in training data.

---

### Experiment 7: Probing Classifier on Hidden States
**Goal:** Can a linear model recover numeric magnitude from the model's hidden representation at a number's token position?

**Method:**
1. Collect hidden states at numeric token positions across the test set.
2. Label each with the log-magnitude of the number (log scale to handle range).
3. Train a linear regression probe on 80% of examples, evaluate on 20%.
4. Compare R² across layers (early vs. middle vs. final).

**Expected result:** Low R² across all layers, meaning the model does not preserve magnitude in its representations. If middle layers show higher R² than final layers, that would suggest the information is present but gets discarded during task-specific processing.

---

### Experiment 8: Out-of-Distribution Number Test
**Goal:** Measure model confidence on inputs containing numbers never seen in training.

**Method:**
1. Construct synthetic inputs using extreme or implausible financial values (e.g., EPS of `$84,201.33`, revenue decline of `0.00001%`).
2. Run inference and measure output confidence (logit spread or entropy).
3. Compare confidence to matched inputs with in-distribution numbers.

**Expected result:** Similar confidence regardless of whether numbers are plausible, indicating context-driven rather than value-driven inference.
...
| H4: Fine-tuning doesn't fix it | Fine-tuned attention on numerics ≈ base model | Fine-tuned model shows >2x attention weight on numeric tokens |
| H5: Embedding blindness | Numeric embeddings do not cluster by magnitude (R² < 0.1 in probe) | Clear ordinal structure in embedding space |
